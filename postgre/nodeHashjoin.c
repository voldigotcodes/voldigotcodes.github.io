 ...
 * 
 * Ce fichier a été modifé par le groupe de Voldi, Urbain et Yacine pour la classe de CSI 3530
 *
 *-------------------------------------------------------------------------
 */
...
ExecHashJoin(HashJoinState *node)		/*CSI 3530: Ajouter par notre groupe : altered*/
{
	EState	   *estate;
	PlanState  *outerNode;
	HashState  *inner_hashNode;	/*CSI 3530: Ajouter par notre groupe :new*/
	HashState  *outer_hashNode; /*CSI 3530: Ajouter par notre groupe :new*/
	List	   *joinqual;
	List	   *otherqual;
	TupleTableSlot *inntuple;
    TupleTableSlot *outtuple;	/*CSI 3530: Ajouter par notre groupe : new*/ 	
	ExprContext *econtext;
	ExprDoneCond isDone;
	HashJoinTable inner_hashtable;	//*CSI 3530: Ajouter par notre groupe : call hashtable in orginal file
	HashJoinTable outer_hashtable;  /*CSI 3530: Ajouter par notre groupe : new*/
	HeapTuple	curtuple;
	TupleTableSlot *outerTupleSlot;
	TupleTableSlot *innerTupleSlot;	/*CSI 3530: Ajouter par notre groupe :new*/
	uint32		hashvalue;
	int			batchno;
...
*/
	...
	if (inner_hashtable == NULL && outer_hashtable == NULL)
	{
		 ...
		 */
/*	CSI 3530: Ajouter par notre groupe : we have no need for an early finish as we have no blocking effect	
...
//*CSI 3530: Ajouter par notre groupe 		(void) MultiExecProcNode((PlanState *) hashNode);

		/*
		 * If the inner relation is completely empty, and we're not doing an
		 * outer join, we can quit without scanning the outer relation.
		 */
//*CSI 3530: Ajouter par notre groupe 		if (hashtable->totalTuples == 0 && node->js.jointype != JOIN_LEFT)
//*CSI 3530: Ajouter par notre groupe 			return NULL;

		/*
		 * need to remember whether nbatch has increased since we began
		 * scanning the outer relation
		 */
//		hashtable->nbatch_outstart = hashtable->nbatch;  //CSI 3530: Ajouter par notre groupe : check
		 ...
		 * set it again.)
		 */
//		node->hj_OuterNotEmpty = false;		//CSI 3530: Ajouter par notre groupe :check
	}
...
//				printf("got new inner \n");
	...
	//get hash value
				...
				hashvalue = ExecHashGetHashValue(outer_hashtable, econtext,		//CSI 3530: Ajouter par notre groupe : trying to get the hash value in the outter hash table
...
//get hash value
			...
			hashvalue = ExecHashGetHashValue(inner_hashtable, econtext,				//CSI 3530: Ajouter par notre groupe 
...
//CSI 3530: Ajouter par notre groupe 
