 ...
 *		ExecHash
 *
 *		Cette fonction est basée sur le fonction de MultiExecHash.
 *		Elle permet de faire le hashage et l'ajout dans la table 
 *		de hachage.
 * ----------------------------------------------------------------
 */
...
ExecHash(HashState *node)
{
    ...
    /* si le tuple est null on retourne NULL */
	if (TupIsNull(slot))
      return NULL;
    hashtable->totalTuples += 1;
    /* recupération du slot en fonction du type de plan (inner ou outer) */
    if (node->isInner) {
					...
					uint32 hashvalue)
{
    ...
    elog(NOTICE, "%d bucket no", bucketno);
}
...
ExecScanHashBucket(HashJoinState *hjstate,
				   ExprContext *econtext,
                   HashJoinTable phashtable) /* [CSI3530] */
{
	List	   *hjclauses = hjstate->hashclauses;
	HashJoinTable hashtable = phashtable; /* [CSI3530] */
